...
class WebGPUProgram(Program['WebGpuDevice']):
  ...
  def __call__(self, *bufs:webgpu.WGPUBuffer, global_size:tuple[int,int,int]=(1,1,1), local_size:tuple[int,int,int]=(1,1,1),
    ...
    bind_entries = (webgpu.WGPUBindGroupLayoutEntry * (1+len(bufs)+len(vals)))(
      bgl_entry(0, 'Uniform'), *(bgl_entry(i+1, 'Uniform' if slot >= len(bufs) else 'Storage') for i, (_,slot,*_) in enumerate(self.signature)))
    ...
    args = [(bufs+vals)[slot] for _,slot,*_ in self.signature]
    bindings = (webgpu.WGPUBindGroupEntry * (1+len(bufs)+len(vals)))(bg_entry(0, float('inf')), *(bg_entry(i+1, x) for i,x in enumerate(args)))
